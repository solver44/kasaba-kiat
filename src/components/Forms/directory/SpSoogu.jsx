...
function FormBody({ isOpen, ...props }) {
  ...
  return (
    ...
    <Label className={props.inline ? "bp4-inline" : ""}>
      ...
      <Label className={props.inline ? "bp4-inline" : ""}>
        {translateKey("code_on")} <span className="bp4-text-muted">*</span>
        <InputGroupWithMessage
            fill={true}
            onChange={handleChange}
            name={"code_on"}
            value={formValues.code_on}
            intent={errorMessage.code_on ? Intent.DANGER : intent}
            large={properties.inputLarge}
            message={errorMessage.code_on}
          />
      </Label>
      <Label className={props.inline ? "bp4-inline" : ""}>
...
export default function SpSooguForm(props) {
  const { isOpen, handleClose, isLoading, is_active, saveRow, error, data, ...inprops } =
    props;
